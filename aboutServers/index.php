...
if ($getServerByLink->num_rows < 1) {
    header("Location: " . BASEURL . "/notfound");
    exit;
}

$fetchServer = $getServerByLink->fetch_assoc();
$serverId = $fetchServer['server_id'];
$ownerServer = getUser($fetchServer['created_by']);
$title = "Server | " . $fetchServer['server_name'];

$isValidMember = $db->query("SELECT * FROM server_members WHERE server_id = '$serverId' AND member_id = '$decryptedUserId'");

if ($isValidMember->num_rows > 0) {
    header("Location: " . BASEURL . "/servers/?v=$serverLink");
    exit;
}

if (isset($_POST['joinServer'])) {
    if (joinServer($fetchServer['server_id'], $decryptedUserId)) {
        header("Location: " . BASEURL . "/servers/?v=$serverLink");
        exit;
    } else {
...
if (isset($_POST['requestJoin'])) {
    if (requestToJoinServer($serverId, $decryptedUserId, $_POST['reason']) > 0) {
        header("refresh:0;url= ");
        exit;
    } else {
        header("refresh:0;url= ");
    ...
    <div class="col-12 col-lg-6 m-auto border rounded-4 p-3">
        <div class="title-content d-flex">
            <div class="m-auto d-flex gap-2 pb-3">
                <img src="<?= BASEURL ?>/favicon.ico" alt="<?= $fetchServer['server_name'] ?> server profile" class="d-block m-auto rounded-3">
                <p class="display-5 m-auto"><?= $fetchServer['server_name'] ?></p>
            </div>
        </div>

        <?php
        if ($fetchServer['server_visibility'] === "private") {
        ?>
        ...
        <?php
        } elseif ($fetchServer['server_visibility'] === "public") {
        ?>
        ...
        <div class="main-content">
            <?php if ($ownerServer === NULL) { ?>
                <p class="fs-6 mb-0">This server was created by <span class="fw-bold">@DELETED USER</span><span class="text-muted fw-sm"> on <?= $fetchServer['created_at'] ?></span></p>
            <?php } elseif ($ownerServer['username'] != NULL) { ?>
                <p class="fs-6 mb-0">This server was created by <span class="fw-bold">@<?= $ownerServer['username'] ?></span><span class="text-muted fw-sm"> on <?= $fetchServer['created_at'] ?></span></p>
            <?php } ?>
            <p class="fs-6">Description: <?= $fetchServer['server_description'] != NULL ? $fetchServer['server_description'] : "-" ?></p>
        </div>
                ...
                <div class="modal-content">
                    ...
                    <form action="" method="post">
                        <div class="modal-body">
                            <span class="fw-bold"><?= $fetchServer['server_name'] ?></span> server is private, please fill in the column below why you want to join the server
                            <div class="mb-3">
                    ...
                    <form action="" method="post">
                        <div class="modal-body">
                            <div class="card border border-0 p-2 text-center">
                                ...
                                <div> <small class="text-muted">ask the server admin to get the server code.</small> </div>
